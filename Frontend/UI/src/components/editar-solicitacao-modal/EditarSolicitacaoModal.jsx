...
export function ModalEditarSolicitacao({ solicitacao, obras, equipamentos, tecnicosCadastrados, aoFechar, aoSalvar }) {
  ...
  const alterarQuantidade = (item, diferenca) => definirItens((atuais) => atuais.map((atual) => {
    ...
    const limite = grupoPorChave(item.chave)?.disponivel || 1;
  ...
  const materiais = itens.flatMap((item) => {
  ...
  const itensValidos = itens.length > 0 && itens.every((item) => {
    const grupo = grupoPorChave(item.chave);
    return grupo && item.quantidade >= 1 && item.quantidade <= grupo.disponivel;
  });
  ...
  return <EstruturaModal titulo="Editar solicitação" subtitulo={`Movimentação · ${String(solicitacao.id).toUpperCase()}`} aoFechar={aoFechar} largo>
    <div className={styles.formulario}>
      ...
      <section className={styles.secaoMateriais}>
        <header><div><strong>Itens solicitados</strong><small>A quantidade não pode ultrapassar o saldo disponível na origem.</small></div><b>{itens.length}</b></header>
        <div className={styles.listaMateriais}>
          {itens.map((item) => {
            ...
            return <div className={`${styles.material} ${!grupo ? styles.materialPendente : ''}`} key={item.chave}>
              ...
              <div className={styles.nomeItem}><strong>{item.nome}</strong>{grupo ? <small>{grupo.tipo} · {grupo.disponivel} disponível</small> : <small>Sem disponibilidade na origem selecionada</small>}</div>
              <div className={styles.quantidade}><button type="button" disabled={!grupo || item.quantidade <= 1} onClick={() => alterarQuantidade(item, -1)} aria-label={`Diminuir quantidade de ${item.nome}`}><Minus /></button><b>{item.quantidade}</b><button type="button" disabled={!grupo || item.quantidade >= grupo.disponivel} onClick={() => alterarQuantidade(item, 1)} aria-label={`Aumentar quantidade de ${item.nome}`}><Plus /></button></div>
        ...
        <div className={styles.novoItem}>
          <label><span>Adicionar novo item</span><select className={styles.campo} value={modeloParaAdicionar} onChange={(evento) => definirModeloParaAdicionar(evento.target.value)}><option value="">Selecione um item disponível</option>{opcoesParaAdicionar.map((grupo) => <option key={grupo.chave} value={grupo.chave}>{grupo.nome} · saldo {grupo.disponivel}</option>)}</select></label>
          <button type="button" onClick={adicionarItem} disabled={!modeloParaAdicionar}><Plus /> Adicionar</button>
        </div>
        {!gruposDisponiveis.length && <p className={styles.aviso}>Não existem itens disponíveis na origem selecionada.</p>}
      </section>
