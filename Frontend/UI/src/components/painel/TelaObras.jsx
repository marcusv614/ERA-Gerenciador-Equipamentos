  ...
  return <div className={estilos.obraList}>{obras.map((obra) => {
    ...
    return <div key={obra.id} className={estilos.obraCard}>
      <div className={estilos.obraHead}>
        ...
        <div className={estilos.obraActionsGroup}><span className={estilos.obraCount}>{equipamentosDaObra.length} equip.</span><div className={estilos.obraExportActions}><button type="button" className={estilos.obraExportBtn} onClick={() => aoImprimirCautela(obra)}><FileText size={13} /> Inventário da obra</button><button type="button" className={estilos.obraExportBtnSecondary} onClick={() => aoImprimirHistorico(obra)}><Download size={13} /> Histórico da obra</button></div></div>
      </div>
