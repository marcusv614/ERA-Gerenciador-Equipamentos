...
export function TelaAlterarSenha() {
  ...
  return <main className={estilos.pagina}><section className={estilos.cartao}>
    ...
    <form onSubmit={enviar} className={estilos.formulario}>
      ...
      <label>Nova senha<div className={estilos.campo}><input type={senhasVisiveis?'text':'password'} autoComplete="new-password" minLength={8} maxLength={128} value={novaSenha} onChange={(evento)=>definirNovaSenha(evento.target.value)} required/></div></label>
      <label>Confirmar nova senha<div className={estilos.campo}><input type={senhasVisiveis?'text':'password'} autoComplete="new-password" minLength={8} maxLength={128} value={confirmacao} onChange={(evento)=>definirConfirmacao(evento.target.value)} required/><button type="button" className={estilos.visualizarSenha} onClick={()=>definirSenhasVisiveis((visivel)=>!visivel)} aria-label={rotuloVisibilidade} title={rotuloVisibilidade}>{senhasVisiveis?<EyeOff size={18}/>:<Eye size={18}/>}</button></div></label>
