...
export function ModalNovoUsuario({ funcionarios, aoFechar, aoSalvar, apenasTecnicos = false }) {
  ...
  const valido = (!apenasTecnicos || dados.funcionarioId) && dados.nome.trim() && dados.login.trim() && dados.senhaTemporaria.length >= 8;
  ...
  return <EstruturaModal titulo="Novo usuário" subtitulo="Conceda acesso ao painel e defina as permissões" aoFechar={aoFechar}><div className={styles.form}>
    <CampoFormulario rotulo={apenasTecnicos ? 'Técnico cadastrado' : 'Funcionário vinculado (opcional)'}><select className={styles.input} value={dados.funcionarioId} onChange={(evento) => selecionarFuncionario(evento.target.value)}><option value="">{apenasTecnicos ? 'Selecione o técnico' : 'Sem vínculo'}</option>{funcionarios.map((funcionario) => <option key={funcionario.id} value={funcionario.id}>{funcionario.nome}</option>)}</select></CampoFormulario>
    ...
    <div className={styles.grid2}>
      <CampoFormulario rotulo="Perfil"><select disabled={apenasTecnicos} className={styles.input} value={dados.perfil} onChange={(evento) => alterar('perfil', evento.target.value)}><option value="TECNICO">Técnico</option><option value="ESTOQUE">Estoquista</option><option value="GERENTE">Gerente</option><option value="ADMIN">Administrador</option></select></CampoFormulario>
      <CampoFormulario rotulo="Senha temporária (mínimo 8)"><div style={{ display: 'flex', alignItems: 'center' }}><input type={senhaVisivel ? 'text' : 'password'} autoComplete="new-password" minLength={8} maxLength={128} className={styles.input} value={dados.senhaTemporaria} onChange={(evento) => alterar('senhaTemporaria', evento.target.value)} /><button type="button" onClick={() => definirSenhaVisivel((visivel) => !visivel)} aria-label={rotuloVisibilidade} title={rotuloVisibilidade} style={{ width: 40, height: 40, marginLeft: -44, border: 0, background: 'transparent', cursor: 'pointer' }}>{senhaVisivel ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></CampoFormulario>
    </div>
