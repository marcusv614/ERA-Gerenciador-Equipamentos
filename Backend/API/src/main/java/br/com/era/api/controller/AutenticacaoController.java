...
public class AutenticacaoController {
    ...
    @PostMapping("/login") public AutenticacaoDto.Sessao login(@Valid @RequestBody AutenticacaoDto.Login dados,HttpServletRequest request,HttpServletResponse response){
        try{Authentication auth=authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(dados.login(),dados.senha()));request.getSession(true);request.changeSessionId();SecurityContext context=SecurityContextHolder.createEmptyContext();context.setAuthentication(auth);SecurityContextHolder.setContext(context);new HttpSessionSecurityContextRepository().saveContext(context,request,response);Usuario usuario=usuarios.registrarSucesso(auth.getName());return usuarios.sessao(usuario);}catch(AuthenticationException e){usuarios.registrarFalha(dados.login());throw new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Login ou senha inválidos.");}
    }
