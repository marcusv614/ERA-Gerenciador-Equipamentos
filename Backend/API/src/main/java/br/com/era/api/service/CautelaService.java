...
public class CautelaService {
    ...
    @Transactional(readOnly=true) public List<CautelaDto.Resposta> listar(Long solicitacaoId,String login){Usuario usuario=usuarios.buscarPorLogin(login);List<Cautela> cautelas=solicitacaoId==null?repository.findAllByOrderByEmitidaEmDesc():repository.findBySolicitacaoIdOrderByEmitidaEmDesc(solicitacaoId);if(usuario.getPerfil()==PerfilUsuario.TECNICO)cautelas=cautelas.stream().filter(c->usuario.getFuncionario()!=null&&c.getSolicitacao().getTecnico().getId().equals(usuario.getFuncionario().getId())).toList();return cautelas.stream().map(this::resposta).toList();}
