...
public class SolicitacaoService {
    ...
    @Transactional(readOnly=true) public List<SolicitacaoDto.Resposta> listar(){return repository.findAll().stream().filter(s->"Movimentação".equals(s.getTipo())).map(this::resposta).toList();}
    @Transactional public SolicitacaoDto.Resposta cadastrar(SolicitacaoDto.Requisicao d){Solicitacao s=new Solicitacao();s.setTipo("Movimentação");s.setStatus("Pendente");s.setTecnico(funcionarios.buscarPorNome(d.tecnico()));s.setSolicitante(funcionarios.buscarPorNome(d.solicitante()==null||d.solicitante().isBlank()?d.tecnico():d.solicitante()));s.setObraOrigem(obras.buscarOpcional(d.obraOrigemId()));s.setObraDestino(obras.buscarOpcional(d.obraDestinoId()));s.setDataSolicitacao(d.dataSolicitacao());s.setObservacao(d.observacao());s.substituirMateriais(criarMateriais(d.materiais()));return resposta(repository.save(s));}
    ...
    @Transactional public SolicitacaoDto.Resposta aprovar(Long id){Solicitacao s=buscar(id);if(!"Pendente".equals(s.getStatus()))throw new RegraNegocioException("A solicitação já foi analisada.");sincronizarMovimentacoes(s);s.setStatus("Aprovada");s.setDataDecisao(LocalDate.now());return resposta(s);}
